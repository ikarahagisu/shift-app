...
if not valid_staff.empty:
    ...
    for t_idx, doc_name in enumerate(doctor_names):
        ...
        with tabs[t_idx]:
            ...
            st.markdown("<div style='color:#bf5700;background:#fff0c2;border:1px solid #ef9b20;border-radius:6px;padding:8px 10px;font-size:0.9rem;font-weight:600;'>⚠は 翌日PMにdutyがあるため、原則として宿直を外す曜日です。ただし、翌日が休日の場合は宿直に入ることがあります。</div>", unsafe_allow_html=True)
