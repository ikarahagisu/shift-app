...
with st.expander("📖 初めての方へ：このアプリの特徴と使い方（クリックして開く）", expanded=False):
    ...
    **2. スタッフ条件の読み込み・入力（必須）**
    「📥 ひな形（CSV）」をダウンロードしてExcelで入力しアップロードするか、画面上の表を直接クリックして編集してください。
    * **【NG日】** 先生ごとにタブを切り替え、カレンダーから休みたい日をポチポチ選んで**【確定する】**を押します。
    * **【希望日】** 入りたい日を「10, 15」や「10:宿直A, 15:日直B」のように入力します。
    * **【希望優先度】** 希望を通す「強さ」です。`100` 以上にすると、回数上限などのルールを無視して**【確実】**にそのシフトに入ります。
    * **【各種ルール】** 「最低空ける日数」「月間最小/最大回数」「各シフト枠の上限」を人ごとに設定できます。

    **3. 過去・決定済みシフトの入力（任意）**
    先月分のシフト表や、今月の「一部だけ確定させたシフト」があれば入力します。残りの空き枠だけをAIが綺麗に埋めてくれます。

    **4. シフトの自動生成**
    一番下の「🚀 このデータでシフトを自動生成する」ボタンを押します。完成した表や、勤務回数の実績レポートはCSVでダウンロードできます。
    """)
