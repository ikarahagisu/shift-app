...
with st.expander("📖 初めての方へ：このアプリの使い方マニュアル（クリックして開く）", expanded=False):
    ...
    ※行の追加は表の右上の「＋」アイコンから。**行の削除は、消したい行の一番左端のマスをクリックして選択し、キーボードの「Delete」キーを押してください。**
...
st.markdown("👇 以下の表のセルをクリックして直接編集するか、CSVファイルをアップロードしてください。<br>（※行の追加は右上の「＋」アイコンから。**行の削除は、消したい行の一番左端のマスをクリックして選択し、キーボードの「Delete」キーを押してください**）", unsafe_allow_html=True)
...
fixed_cols = ["日付", "平日or休日", "宿直A", "宿直B", "外来宿直", "日直A", "日直B", "外来日直"]
...
def generate_shift(target_year, target_month, staff_df, custom_holidays, fixed_df=None):
    ...
    if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
        ...
        for d in range(1, num_days + 1):
            ...
            row = {"日付": f"{target_month}/{d}({weekday_ja[date_obj.weekday()]})", "平日or休日": day_str}
                ...
                if success:
                    ...
                    for doc in doctors_list:
                        ...
                        for s in shift_columns:
                            ...
                            doc_data[s] = count
                            total_count += count
                            hol_count += (df_result[df_result['平日or休日'] == '休日'][s] == doc).sum()
