...
st.caption("セルの上段は日直、下段は宿直です。NGの側だけ色が付きます。⚠は原則、宿直を外す曜日です。「可」はその勤務帯にNG指定がない意味で、曜日・回数など他の条件は適用されます。")
